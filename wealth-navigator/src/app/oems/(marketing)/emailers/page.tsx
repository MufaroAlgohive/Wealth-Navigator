...
/**
 * Marketing › Emailers — embedded email-app surface (campaigns + webhook
 * triggers + send logs). The legacy `/admin/emailers` page was redirected to
 * `/oems/marketing/emailers` (Mint OEM finalisation Phase A1 + B7) and the
 * live component moved here so the marketing hub owns the full emailer
 * surface per Lonwabo's preference (EMAIL_APP_MODE=embedded).
 */

import { Copy, Info, Plus, RefreshCw } from "lucide-react";
        ...
        <TabsContent value="triggers" className="space-y-4">
          <div className={cardClass}>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Copy this URL into Supabase → Database → Webhooks. Point it at the tables below and Mint will
              send emails on database events.
            </p>
            ...
            <div className="mt-3 flex items-start gap-2 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2.5 text-xs text-foreground/80">
              ...
              <span>
                In Supabase set the URL, choose tables (e.g. <code>stock_holdings</code>,{" "}
                <code>profiles</code>, <code>wallet_transactions</code>), select events, and add header a
                webhook secret header value from your Supabase webhook configuration.
              </span>
            </div>
          </div>
          ...
          <div className={cardClass}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Each maps a Supabase table event to an email type.
                </p>
              </div>
            ...
            {triggers === null && !triggersError ? (
              ...
              <p className="py-8 text-center text-xs text-muted-foreground">
                No triggers yet. Click “Add trigger”.
              </p>
            ) : (
                    ...
                    {triggers?.map((t) => (
                      <tr key={t.id} className="border-b border-border/50 last:border-b-0 hover:bg-accent/30">
                        <td className={tdClass}>
                          ...
                          {t.description && (
                            <div className="mt-0.5 text-[10.5px] text-muted-foreground">{t.description}</div>
                          )}
                          {t.condition_field && (
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Event">
                <Select
                  ...
                  onValueChange={(v) =>
                    setForm((f) => ({ ...f, event_type: v as TriggerForm["event_type"] }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Email type">
                <Select
                  ...
                  onValueChange={(v) => setForm((f) => ({ ...f, email_type: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
