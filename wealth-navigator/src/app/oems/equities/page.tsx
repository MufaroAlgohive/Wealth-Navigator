  ...
  return (
    <div className="space-y-3">
      ...
      <Panel
        ...
        right={<Pill tone="success" size="xs" dot>LIVE · L1</Pill>}
      >
        {equitiesQ.isLoading ? (
          ...
          <table className="w-full font-mono text-xs">
            ...
            <tbody className="divide-y divide-border/60">
              {equities.map((e) => {
                const ref = seedLastFor(e.symbol);
                return (
                  <tr key={e.symbol} className="hover:bg-muted/30">
                    <td className="px-2.5 py-1.5 font-semibold">{e.symbol}</td>
                    ...
                    <td className="px-2.5 py-1.5 text-right tabular-nums">
                      <NumberCell sym={e.symbol} fallback={ref} decimals={2} />
                    </td>
                    ...
                    <td className="px-2.5 py-1.5 text-right tabular-nums">{formatNumber(ref)}</td>
                    <td className="px-2.5 py-1.5 text-right tabular-nums text-muted-foreground">
                      {formatNumber(1_000_000 + Math.random() * 5_000_000)}
                    </td>
                    <td className="px-2.5 py-1.5 text-right">
                      <NumberCell sym={e.symbol} fallback={ref} decimals={2} showChange size="xs" />
                    </td>
                  </tr>
                );
              })}
