...
interface Counts { total?: number; public?: number; featured?: number; users?: number; kyc?: number; bank?: number; }
interface Featured { id: string; name: string | null; short_name: string | null; sector: string | null; holdings: number; ytd: number | null; }

const PERIODS: [string, string][] = [["1D", "1d_pct"], ["5D", "5d_pct"], ["1M", "1m_pct"], ["6M", "6m_pct"], ["YTD", "ytd_pct"], ["1Y", "1y_pct"], ["5Y", "5y_pct"], ["All", "all_pct"]];
...
export default function DashboardPage() {
  ...
  React.useEffect(() => {
    fetch("/api/admin/dashboard").then((r) => r.json()).then((d) => {
      if (d.ok) { setCounts(d.counts || {}); setFeatured(d.featured || []); setAssetReturns(d.assetReturns || []); setStrategyReturns(d.strategyReturns || []); }
  ...
  const chartData = React.useMemo(() =>
    strategyReturns.map((r) => ({ name: String(r.name ?? "—").slice(0, 12), ytd: num(r.ytd_pct) ?? 0 })).sort((a, b) => b.ytd - a.ytd).slice(0, 8),
    [strategyReturns]);
    ...
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          {chartData.length === 0 ? <p className="py-10 text-center text-xs text-muted-foreground">No strategy returns.</p> : (
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} layout="vertical" margin={{ left: 8, right: 16, top: 4, bottom: 4 }}>
                  <XAxis type="number" hide /><YAxis type="category" dataKey="name" width={90} tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }} />
                  <Tooltip formatter={(v: number) => `${v.toFixed(2)}%`} contentStyle={{ fontSize: 11 }} />
                  <Bar dataKey="ytd" radius={[0, 4, 4, 0]}>{chartData.map((d, i) => <Cell key={i} fill={d.ytd >= 0 ? "hsl(var(--success))" : "hsl(var(--destructive))"} />)}</Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
            ...
            <div className="space-y-2">
              {featured.map((f) => (
                <Link key={f.id} href={`/admin/factsheets?id=${f.id}` as Route} className="flex items-center justify-between rounded-xl border border-border px-3 py-2.5 hover:border-primary/40">
                  <div className="min-w-0"><p className="truncate text-sm font-semibold text-foreground">{f.name}</p><p className="text-[11px] text-muted-foreground">{f.sector || "—"} · {f.holdings} holdings</p></div>
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        ...
        <Tabs value={period} onValueChange={setPeriod}>
          <TabsList className="flex-wrap">
            {PERIODS.map(([label, key]) => <TabsTrigger key={key} value={key}>{label}</TabsTrigger>)}
          </TabsList>
          <TabsContent value={period} className="mt-4">
            {alerts.length > 0 && (
              <div className="mb-4 rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-foreground/90">
                ⚠ {alerts.length} {mode === "assets" ? "assets" : "strategies"} down ≥ 4%: {alerts.slice(0, 6).map((a) => a.label).join(", ")}{alerts.length > 6 ? "…" : ""}
              </div>
            )}
            {ranked.length === 0 ? <p className="py-10 text-center text-xs text-muted-foreground">No return data for this period.</p> : (
...
function BarList({ title, items, maxAbs, positive }: { title: string; items: { label: string; pct: number }[]; maxAbs: number; positive: boolean }) {
  return (
    <div>
        ...
        {items.map((it) => (
          <div key={it.label} className="flex items-center gap-2">
            ...
            <div className="h-2 flex-1 overflow-hidden rounded-full bg-muted">
              <div className={cn("h-full rounded-full", positive ? "bg-success" : "bg-destructive")} style={{ width: `${Math.min(100, (Math.abs(it.pct) / maxAbs) * 100)}%` }} />
            </div>
            <span className={cn("w-16 shrink-0 text-right text-xs font-semibold", pctCls(it.pct))}>{pctStr(it.pct)}</span>
          </div>
        ))}
      </div>
    </div>
...
function Kpi({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-border bg-card px-4 py-3"><div className="text-[10.5px] font-semibold uppercase tracking-wider text-muted-foreground">{label}</div><div className="mt-1 text-2xl font-bold text-foreground">{value}</div></div>;
}
