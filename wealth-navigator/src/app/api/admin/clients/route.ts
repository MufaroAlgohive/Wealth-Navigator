...
import { can, getAdminContext, isAdminRole } from "@/lib/admin/rbac";
...
export function deriveKyc(
  ob?: {
    ...
    sumsub_review_answer?: string | null;
    sumsub_review_status?: string | null;
  },
  ra?: { kyc_verified?: boolean | null; kyc_needs_resubmission?: boolean | null },
  pack?: unknown,
): KycState {
  const status = String(ob?.kyc_status || "")
    .trim()
    .toLowerCase();
  const answer = String(ob?.sumsub_review_answer || "")
    .trim()
    .toLowerCase();
  const review = String(ob?.sumsub_review_status || "")
    .trim()
    .toLowerCase();
  ...
  const packAnswer = String(
    packResult.reviewAnswer || packReview.reviewAnswer || packRecord.reviewAnswer || "",
  )
    .trim()
    .toLowerCase();
  const packStatus = String(packReview.reviewStatus || packRecord.reviewStatus || packRecord.status || "")
    .trim()
    .toLowerCase();
  ...
  if (
    packAnswer === "yellow" ||
    packAnswer === "orange" ||
    (!packAnswer && /pending|init|review|process/.test(packStatus))
  )
    return "pending";
...
function findProviderValue(source: unknown, aliases: string[], depth = 0): unknown {
  ...
  for (const [key, value] of Object.entries(source as Record<string, unknown>)) {
    if (
      wanted.has(key.replace(/[^a-z0-9]/gi, "").toLowerCase()) &&
      value != null &&
      value !== "" &&
      typeof value !== "object"
    )
...
function buildRichDetails(
  ...
  const experianArchive = archiveRows
    .map((row) => parseRecord(row.resource_metadata))
    .filter((metadata) => {
      const marker = `${String(metadata.provider || "")} ${String(metadata.source || "")}`.toLowerCase();
      return marker.includes("experian");
    });
  const retainedKyc =
    raw.experian_kyc_addresses || raw.experian_kyc_contact || experianArchive.length
      ? {
          addresses: raw.experian_kyc_addresses,
          contact: raw.experian_kyc_contact,
          stats: raw.experian_kyc_stats,
          archive: experianArchive,
        }
      : null;
  const experianKyc = raw.experian_kyc_result ?? parseRecord(pack.experian).kyc ?? retainedKyc;
  const experianIdmn =
    raw.experian_idmn_result ?? parseRecord(pack.experian).idmn ?? pack.experian_idmn ?? null;
  ...
  const idNumber = choose(
    [profile.id_number, "Profile"],
    [info.idNumber ?? findProviderValue(info, ["idNumber", "documentNumber"]), "SumSub"],
    [findProviderValue(experian, ["identityNumber", "idNumber", "documentNumber"]), "Experian"],
  );
  const storedGenderSource = String(provenance.gender || "")
    .toLowerCase()
    .includes("derived")
    ? "Derived from SA ID"
    : "SumSub";
  const explicitGender = choose(
    [profile.gender, "Profile"],
    [info.gender, storedGenderSource],
    [findProviderValue(experian, ["gender", "sex"]), "Experian"],
  );
  const inferredGender = inferGenderFromSouthAfricanId(idNumber.value);
  const gender =
    explicitGender.value != null
      ? explicitGender
  ...
  return {
    fields: {
      first_name: choose(
        ...
        [info.firstName ?? info.firstNameEn, "SumSub"],
        [findProviderValue(experian, ["firstName", "first_name", "forename", "givenName"]), "Experian"],
      ),
      last_name: choose(
        [profile.last_name, "Profile"],
        [info.lastName ?? info.lastNameEn, "SumSub"],
        [findProviderValue(experian, ["lastName", "last_name", "surname", "familyName"]), "Experian"],
      ),
      email: choose(
        [profile.email, "Profile"],
        [info.email, "SumSub"],
        [findProviderValue(experian, ["email", "emailAddress"]), "Experian"],
      ),
      phone: choose(
        [profile.phone_number, "Profile"],
        [info.phone ?? pack.phone, "SumSub"],
        [findProviderValue(experian, ["phoneNumber", "mobileNumber", "cellphone", "cell"]), "Experian"],
      ),
      date_of_birth: choose(
        [profile.date_of_birth, "Profile"],
        [info.dob ?? info.dateOfBirth, "SumSub"],
        [findProviderValue(experian, ["dateOfBirth", "birthDate", "dob"]), "Experian"],
      ),
      gender,
      id_number: idNumber,
      address: choose(
        [profile.address, "Profile"],
        [findProviderValue(info, ["formattedAddress", "residentialAddress", "streetAddress"]), "SumSub"],
        [
          findProviderValue(experianKyc ?? experian, [
            "formattedAddress",
            "formatted",
            "residentialAddress",
            "streetAddress",
      ...
      employer: choose(
        [onboarding.employer_name, "Onboarding"],
        [findProviderValue(info, ["employerName", "employer"]), "SumSub"],
        [findProviderValue(experianKyc ?? experian, ["employerName", "employer"]), "Experian"],
      ),
      employment_status: choose(
        [onboarding.employment_status, "Onboarding"],
        [findProviderValue(info, ["employmentStatus", "occupation"]), "SumSub"],
        [findProviderValue(experianKyc ?? experian, ["employmentStatus", "occupation"]), "Experian"],
      ),
    },
    providers: {
      profile: Object.keys(profile).length > 0,
      sumsub: Boolean(pack.info || pack.fixedInfo),
      experian: Boolean(experianKyc || experianIdmn),
    },
  };
}
...
async function resolveCertificateUrl(
  ...
  if (raw.startsWith("storage://")) {
    ...
    if (slash > 0) {
      bucket = pointer.slice(0, slash);
      path = pointer.slice(slash + 1);
    }
    ...
    try {
      ...
      if (index >= 0) {
        ...
        } else if (parts[0] === "sign" && parts.length >= 4) {
          bucket = parts[2] ?? "";
          path = parts.slice(3).join("/");
        }
      }
    } catch {
      /* retain the original URL below */
    }
  } else {
...
export async function GET(req: Request) {
  ...
  if (action === "list") {
    ...
    const raMap: Record<
      string,
      { kyc_verified?: boolean | null; kyc_needs_resubmission?: boolean | null; bank_linked?: boolean | null }
    > = {};
    ...
    if (ids.length) {
      const [{ data: ob }, { data: ra }, { data: packs }, { data: family }] = await Promise.all([
        db
          ...
          .select("user_id, kyc_status, sumsub_review_answer, sumsub_review_status")
          .in("user_id", ids),
        db
          .from("required_actions")
          .select("user_id, kyc_verified, kyc_needs_resubmission, bank_linked")
          .in("user_id", ids),
    ...
    const profileClients = rows.map((p) => {
      ...
      return {
        ...
        family_role: childProfileIds.has(String(p.id))
          ? "child"
          : parentIds.has(String(p.id))
            ? "parent"
            : "other",
    ...
    const stats = profileClients.reduce(
      (counts, client) => {
  ...
  if (action === "detail") {
    ...
    if (familyMemberId) {
      const [{ data: member }, { data: holds }, { data: txns }] = await Promise.all([
        ...
        db
          ...
          .select("security_id, quantity, avg_fill, Expected_fill, strategy_name_snapshot")
      ...
      const secMap: Record<
        string,
        { symbol: string; name: string | null; last_price: number | null; updated_at?: string | null }
      > = {};
      const intradayMap = new Map<string, number>();
      if (secIds.length) {
        const [{ data: securities }, { data: intraday }] = await Promise.all([
          db.from("securities_c").select("id, symbol, name, last_price, updated_at").in("id", secIds),
          db
        ...
        for (const quote of intraday ?? []) {
          ...
          if (!intradayMap.has(securityId) && Number(quote.current_price) > 0)
            intradayMap.set(securityId, Number(quote.current_price));
        }
        // Yahoo fallback for any child holding with a missing/stale DB price.
        const missingForFallback = (securities ?? [])
        ...
        if (missingForFallback.length > 0) {
          await applyYahooFallback({ rows: missingForFallback, maxYahoo: 40, concurrency: 4 });
          for (const row of missingForFallback as Array<{
            symbol: string;
      ...
      const holdings = (holds ?? []).map((holding) => {
        ...
        const priceCents =
          ...
          (Number(security?.last_price) > 0 ? Number(security?.last_price) : costCents);
        ...
        return {
          ...
          valueCents,
          purchaseValueCents,
          pnlCents: valueCents - purchaseValueCents,
          strategy: holding.strategy_name_snapshot ?? null,
        };
      });
    ...
    const [
      ...
      db
        ...
        .select("resource_metadata,archived_at,file_name")
        .eq("profile_id", userId),
      db
        .from("stock_holdings_c")
        .select("security_id, strategy_id, quantity, avg_fill, Expected_fill, strategy_name_snapshot")
        .eq("user_id", userId)
        .eq("is_active", true)
        .eq("trade_side", "BUY"),
      db
        .from("transactions")
        .select("id, name, description, amount, direction, status, transaction_date")
        .eq("user_id", userId)
    ...
    const { data: linkedChild } = await db
      .from("family_members")
      .select(
        "id,primary_user_id,parent_id,relationship,certificate_url,certificate_verification_status,kyc_status,kyc_reviewed_at",
      )
      .eq("linked_user_id", userId)
      .eq("relationship", "child")
      .maybeSingle();
    const linkedCertificateUrl = linkedChild
      ? await resolveCertificateUrl(db, linkedChild.certificate_url)
      : null;
    ...
    if (secIds.length) {
      ...
      // Yahoo fallback for missing/stale DB prices — keeps the client view live
      // during an IRESS outage. Reads only, never writes back.
      const missingForFallback = (secs ?? [])
        .filter((s) => Number((s as Record<string, unknown>).last_price) <= 0)
        .map((s) => ({
          symbol: s.symbol as string,
          last_price: ((s as Record<string, unknown>).last_price as number | null) ?? null,
    ...
    if (strategyIds.length) {
      ...
      for (const strategy of strategies ?? [])
        strategyMap.set(String(strategy.id), String(strategy.name || ""));
    }
    const holdings = (holds ?? [])
      .map((h) => {
        ...
        const priceCents =
          intradayMap.get(String(h.security_id)) ??
          (sec?.last_price != null && Number(sec.last_price) > 0 ? Number(sec.last_price) : costCents);
        ...
        return {
          ...
          qty,
          valueCents,
          purchaseValueCents: investedCents,
          pnlCents: valueCents - investedCents,
          strategy: h.strategy_name_snapshot ?? strategyMap.get(String(h.strategy_id)) ?? null,
        };
      })
  ...
  if (action === "holdings") {
    ...
    const [{ data: ownHolds }, { data: childHolds }, { data: ownClosed }, { data: childClosed }] =
      await Promise.all([
        db
          .from("stock_holdings_c")
          .select("security_id, strategy_id, quantity, avg_fill, Expected_fill, strategy_name_snapshot")
        ...
        familyMemberIds.length
          ? db
              .from("stock_holdings_c")
              .select(
                "security_id, strategy_id, quantity, avg_fill, Expected_fill, strategy_name_snapshot, family_member_id",
              )
        ...
        db
          .from("stock_holdings_c")
          .select("avg_fill, avg_exit, quantity")
          .eq("user_id", userId)
          .is("family_member_id", null)
          .eq("is_active", false),
        familyMemberIds.length
    ...
    const realizedCents = Math.round(
      closed.reduce((sum, c) => {
        const fill = Number(c.avg_fill) || 0,
          exit = Number(c.avg_exit) || 0,
    ...
    const secMap: Record<
      string,
      { symbol: string; name: string | null; sector: string | null; last_price: number | null }
    > = {};
    const strategyMap = new Map<string, string>();
    const intradayMap = new Map<string, number>();
    if (secIds.length) {
    ...
    const holdings = holds
      .map((h) => {
        ...
        return {
          ...
          pnlCents: valueCents - investedCents,
          strategy:
            (h as { strategy_name_snapshot?: string | null }).strategy_name_snapshot ??
            strategyMap.get(String(h.strategy_id)) ??
            null,
    ...
    const allocations = [...allocationsBySector.entries()].map(([sector, valueCents]) => ({
      sector,
      valueCents,
    }));
  ...
  if (action === "cash") {
    ...
    const [{ data: wallet }, { data: recent }] = await Promise.all([
      ...
      db
        .from("transactions")
        .select(
          "id, name, description, amount, direction, status, transaction_date, broker_fee_cents, isin_fee_cents, transaction_fee_cents",
        )
        .eq("user_id", userId)
        .order("transaction_date", { ascending: false })
        .limit(10),
  ...
  if (action === "sumsub") {
    ...
    if (!sumsubConfigured())
      return NextResponse.json({
        ok: true,
        configured: false,
        notice: "SumSub credentials are not configured.",
      });
    const { data: ob } = await db
      .from("user_onboarding")
      .select("sumsub_external_user_id")
      .eq("user_id", userId)
      .maybeSingle();
...
export async function POST(req: Request) {
  ...
  if (action === "sumsub-refresh-batch") {
    if (!sumsubConfigured())
      return NextResponse.json(
        { ok: false, error: "SumSub credentials are not configured" },
        { status: 503 },
      );
    ...
    if (onboardingError)
      return NextResponse.json({ ok: false, error: onboardingError.message }, { status: 500 });
    const eligible = (onboardingRows ?? []).filter((row) =>
      /^[a-f0-9]{24}$/i.test(String(row.sumsub_applicant_id || "")),
    );
    ...
    for (const row of eligible) {
      ...
      if (!result.ok) {
        results.push({
          user_id: userId,
          status: result.status === 404 ? "not_found" : "failed",
          error: result.error || `SumSub returned ${result.status}`,
        });
        continue;
      }
      ...
      const { error: archiveError } = await db.from("provider_identity_snapshots_c").insert({
        user_id: userId,
        provider: "SUMSUB",
        capture_type: "APPLICANT_BATCH_REFRESH",
        external_reference: String(row.sumsub_applicant_id),
        payload: fetched,
        metadata: { source: "OEM_CLIENT_BATCH_REFRESH" },
        captured_at: refreshedAt,
      });
      if (archiveError) {
        results.push({ user_id: userId, status: "failed", error: `Archive failed: ${archiveError.message}` });
        continue;
      }
      const mergedPack = {
        ...fetched,
        ...existing,
        fixedInfo: Object.keys(parseRecord(fetched.fixedInfo)).length
          ? parseRecord(fetched.fixedInfo)
          : existing.fixedInfo,
        info: { ...fetchedInfo, ...existingInfo },
        review: fetched.review ?? existing.review,
        sumsub_refreshed_at: refreshedAt,
      };
      const { error } = await db
        .from("user_onboarding_pack_details")
        .upsert(
          { user_id: userId, pack_details: mergedPack, updated_at: refreshedAt },
          { onConflict: "user_id" },
        );
      results.push(
        error
          ? { user_id: userId, status: "failed", error: error.message }
          : { user_id: userId, status: "refreshed" },
      );
    }
    return NextResponse.json({
      ok: true,
  ...
  if (action === "sumsub-refresh") {
    ...
    if (!onboarding)
      return NextResponse.json({ ok: false, error: "Client has no onboarding record" }, { status: 409 });
    ...
    if (applicantId && !/^[a-f0-9]{24}$/i.test(applicantId)) {
      return NextResponse.json(
        { ok: false, error: "This record was verified by Experian, mock, or administrative flow—not SumSub" },
        { status: 409 },
      );
    }
    const result = applicantId
      ? await getApplicantById(applicantId)
      ...
      return NextResponse.json(
        {
          ...
          error:
            result.status === 404
              ? "No existing SumSub applicant found"
              : result.error || `SumSub returned ${result.status}`,
        },
    ...
    const { error: archiveError } = await db.from("provider_identity_snapshots_c").insert({
      user_id: userId,
      provider: "SUMSUB",
      capture_type: "APPLICANT_REFRESH",
      external_reference: applicantId || externalUserId,
      payload: fetched,
      metadata: { source: "OEM_CLIENT_REFRESH" },
      captured_at: mergedPack.sumsub_refreshed_at,
    });
    if (archiveError)
      return NextResponse.json(
        { ok: false, error: `SumSub data fetched but immutable archive failed: ${archiveError.message}` },
        { status: 500 },
      );
    const { error } = await db
      .from("user_onboarding_pack_details")
      .upsert(
        { user_id: userId, pack_details: mergedPack, updated_at: new Date().toISOString() },
        { onConflict: "user_id" },
      );
  ...
  if (action === "computershare-number") {
    ...
    const computershareNumber = String(body.computershare_number || "")
      .trim()
      .toUpperCase();
    const password = String(body.password || "");
    if ((!userId && !familyMemberId) || !computershareNumber || !password) {
      return NextResponse.json(
        { ok: false, error: "Client, Computershare number and password are required" },
        { status: 400 },
      );
    }
    ...
    const target = familyMemberId
      ? db
          .from("family_members")
          .update({ computershare_number: computershareNumber, updated_at: new Date().toISOString() })
  ...
  if (action === "child-certificate-review") {
    ...
    const decision =
      body.decision === "approve" ? "verified" : body.decision === "reject" ? "rejected" : "pending";
    if (!familyMemberId)
      return NextResponse.json({ ok: false, error: "family_member_id required" }, { status: 400 });
    let db: ReturnType<typeof createRetailServiceRoleClient>;
    try {
      db = createRetailServiceRoleClient();
    } catch {
  ...
  if (action === "kyc-review") {
    ...
    if (decision === "approve") {
      ...
      await db
        .from("required_actions")
        .update({ kyc_verified: false, kyc_needs_resubmission: true, kyc_pending: false })
        .eq("user_id", userId);
    }
  ...
  return NextResponse.json(
    { ok: false, error: "Document operations are deferred to the storage/backend phase.", deferred: true },
    { status: 501 },
  );
}
