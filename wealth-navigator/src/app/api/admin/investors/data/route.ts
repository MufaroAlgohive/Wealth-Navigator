...
export async function GET() {
  ...
  const { data: holdingsRaw } = await db
    ...
    .select(
      "user_id, family_member_id, security_id, strategy_id, quantity, avg_fill, Expected_fill, market_value, created_at, transaction_id",
    )
    .eq("is_active", true)
    .eq("trade_side", "BUY");
  ...
  const [
    ...
    db
      .from("strategies_c")
      .select("id, name, short_name, description, risk_level, sector")
      .then((r) => r.data ?? []),
    profileIds.length
      ? db
          .from("profiles")
          .select("id, first_name, last_name, email, mint_number, computershare_number")
          .in("id", profileIds)
          .then((r) => r.data ?? [])
      : [],
    secIds.length
      ? db
          .from("securities_c")
          .select("id, symbol, name, sector, logo_url, last_price, change_percent, updated_at")
          .in("id", secIds)
          .then((r) => r.data ?? [])
      : [],
    secIds.length
      ? db
          ...
          .select("security_id, symbol, current_price, ytd_pct, as_of_date")
    ...
    userIds.length
      ? db
          .from("transactions")
          .select(
            "id, user_id, family_member_id, amount, direction, name, description, status, transaction_date, broker_fee_cents, isin_fee_cents, transaction_fee_cents, base_amount_cents, buffer_cents, buffer_consumed_cents",
          )
          .in("user_id", userIds)
    ...
    userIds.length
      ? db
          ...
          .select("user_id, strategy_id, family_member_id, balance_cents")
          .in("user_id", userIds)
          .then((r) => r.data ?? [])
      : [],
    userIds.length
      ? db
          .from("stock_holdings_c")
          .select("user_id, family_member_id, strategy_id, quantity, avg_fill, avg_exit")
          .eq("is_active", false)
          .in("user_id", userIds)
          .then((r) => r.data ?? [])
      : [],
  ...
  for (const [sid, cents] of Object.entries(intraById))
    if (!returnsIds.has(sid)) secLive.push({ security_id: sid, current_price: cents });
  ...
  const liveIds = new Set(secLive.map((row) => String(row.security_id)).filter(Boolean));
  const missingIds = (secMeta as Array<{ id?: string }>)
    .map((s) => String(s.id ?? ""))
    .filter((id) => id && !liveIds.has(id));
  if (missingIds.length > 0) {
    const fallbackRows = missingIds.map((id) => {
      const meta = (
        secMeta as Array<{
          id?: string;
          symbol?: string;
    ...
    for (const row of fallbackRows as Array<{
      ...
      if (row.price_source === "yahoo" && row.last_price != null && row.last_price > 0) {
        ...
        if (id) secLive.push({ security_id: id, current_price: row.last_price });
      }
    }
  }
