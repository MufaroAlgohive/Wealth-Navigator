...
async function requireAdmin() {
  ...
  if (auth.status !== "ok" || !isAdminRole(auth.ctx))
    return { error: NextResponse.json({ ok: false, error: "Admins only" }, { status: 403 }) };
  return { auth };
}
...
export async function GET(req: Request) {
  ...
  if (action === "portfolio") {
    ...
    let holdsQuery = db
      ...
      .select(
        "id,security_id,quantity,avg_fill,Expected_fill,strategy_id,strategy_name_snapshot,transaction_id",
      )
      .eq("user_id", userId)
      .eq("is_active", true)
      .eq("trade_side", "BUY");
    if (familyMemberId) holdsQuery = holdsQuery.eq("family_member_id", familyMemberId);
    else holdsQuery = holdsQuery.is("family_member_id", null);
    ...
    const secIds = [...new Set((holds ?? []).map((h) => h.security_id).filter(Boolean))],
      ...
      > = {},
      intradayMap = new Map<string, number>();
    if (secIds.length) {
      const [{ data: securities }, { data: intraday }] = await Promise.all([
        db.from("securities_c").select("id,symbol,name,logo_url,last_price,updated_at").in("id", secIds),
        db
      ...
      for (const security of securities ?? []) secMap[String(security.id)] = security as never;
      for (const row of intraday ?? []) {
        const id = String(row.security_id);
        if (!intradayMap.has(id) && Number(row.current_price) > 0)
          intradayMap.set(id, Number(row.current_price) / 100);
      }
      // Yahoo fallback: for any security whose DB value is missing/0, resolve
      // a fresh price via Yahoo in-memory so the studio's live MV/PnL keeps
      // rendering during an IRESS outage. Reads only — never writes back.
      // Cents-safe via yahooPriceToCents (.JO verbatim, non-JSE ×100).
      const missingSymbols = Object.values(secMap)
        .filter((s) => Number(s.last_price) <= 0)
        .map((s) => ({
          symbol: s.symbol,
      ...
      if (missingSymbols.length > 0) {
        await applyYahooFallback({ rows: missingSymbols, maxYahoo: 40, concurrency: 4 });
        for (const row of missingSymbols as Array<{
          symbol: string;
          last_price: number | null;
    ...
    const holdings = (holds ?? [])
      .map((h) => {
        const security = secMap[String(h.security_id)],
          ...
          avgRands = (Number(h.avg_fill) || 0) / 100,
          expected = Number(h.Expected_fill) || 0,
          lastPrice = Number(security?.last_price) || 0,
          cost =
            expected > 0 ? (avgRands > 0 && expected > avgRands * 5 ? expected / 100 : expected) : avgRands,
          live = intradayMap.get(String(h.security_id)) ?? (lastPrice > 0 ? lastPrice / 100 : cost),
          marketValue = quantity * live,
    ...
    const realizedTotal = (closed ?? []).reduce((sum, c) => {
      const fill = Number(c.avg_fill) || 0,
        exit = Number(c.avg_exit) || 0,
        qty = Number(c.quantity) || 0;
      return fill && exit && qty ? sum + ((exit - fill) / 100) * qty : sum;
    }, 0);
    const totalValue = holdings.reduce((sum, h) => sum + h.marketValue, 0),
      totalPnl = holdings.reduce((sum, h) => sum + h.pnl, 0) + realizedTotal,
      invested = totalValue - totalPnl;
    let txQuery = db
      .from("transactions")
      .select("id,name,description,amount,direction,status,transaction_date,created_at")
      .eq("user_id", userId)
      .order("created_at", { ascending: false })
      .limit(6);
    if (familyMemberId) txQuery = txQuery.eq("family_member_id", familyMemberId);
    else txQuery = txQuery.is("family_member_id", null);
    const { data: transactions } = await txQuery;
    const strategyMap = new Map<string, { id: string; name: string; value: number; holdings: number }>();
    for (const h of holdings) {
      if (!h.strategyId) continue;
      const id = String(h.strategyId),
        item = strategyMap.get(id) ?? { id, name: String(h.strategy || "Strategy"), value: 0, holdings: 0 };
      if (!h.pending) item.value += h.marketValue;
      item.holdings += 1;
    ...
    let cashCents = 0,
      canonicalWeightedPct = 0,
      canonicalWeight = 0;
    try {
      let retQuery = db
        .from("client_strategy_returns_effective_latest_c")
        .select(
          "strategy_id,basket_value_cents,residual_cash_cents,unused_reserve_cents,inception_pct,ytd_pct",
        )
        .eq("user_id", userId);
      retQuery = familyMemberId
        ? retQuery.eq("family_member_id", familyMemberId)
        : retQuery.is("family_member_id", null);
      const { data: effRows } = await retQuery;
      for (const r of (effRows ?? []) as Array<{
        strategy_id: string;
        basket_value_cents: number | null;
        residual_cash_cents: number | null;
        unused_reserve_cents: number | null;
        inception_pct: number | null;
        ytd_pct: number | null;
      }>) {
        cashCents += (Number(r.residual_cash_cents) || 0) + (Number(r.unused_reserve_cents) || 0);
        const retPct =
          r.inception_pct != null ? Number(r.inception_pct) : r.ytd_pct != null ? Number(r.ytd_pct) : null;
        const weight = Number(r.basket_value_cents) || 0;
        if (retPct != null && Number.isFinite(retPct) && weight > 0) {
          canonicalWeightedPct += retPct * weight;
          canonicalWeight += weight;
        }
      }
    } catch {
      /* cash/return overlay is best-effort; positions-only figures below still render */
    }
    const cashRands = cashCents / 100,
      totalValueWithCash = totalValue + cashRands;
    const pnlPct =
      canonicalWeight > 0
        ? canonicalWeightedPct / canonicalWeight
        : invested > 0
    ...
    return NextResponse.json({
      ok: true,
      holdings,
      transactions: (transactions ?? []).map((t) => ({ ...t, amount: (Number(t.amount) || 0) / 100 })),
      ...
      units: { money: "ZAR", sourcePrices: "ZAc normalized once on server" },
    });
  }
...
export async function POST(req: Request) {
  ...
  const userId = String(body.user_id || ""),
    target = body.target === "live" ? "live" : "dev";
  if (!userId) return NextResponse.json({ ok: false, error: "user_id required" }, { status: 400 });
  const targetUrl = target === "live" ? process.env.MINT_APP_URL_LIVE : process.env.MINT_APP_URL_DEV;
  if (!targetUrl)
    return NextResponse.json(
      { ok: false, error: `MINT_APP_URL_${target.toUpperCase()} is not configured` },
      { status: 503 },
    );
  let db;
  try {
  ...
  if (profileError || !profile?.email)
    return NextResponse.json(
      { ok: false, error: profileError?.message || "Client email not found" },
      { status: 404 },
    );
  ...
  if (error || !actionLink)
    return NextResponse.json(
      { ok: false, error: error?.message || "Could not generate client sign-in link" },
      { status: 500 },
    );
  return NextResponse.json({
    ok: true,
