...
function latestBy<T extends Record<string, unknown>>(rows: T[], key: string): T[] {
  ...
  for (const r of rows) { const k = r[key] as string; if (k && !m.has(k)) m.set(k, r); }
  return [...m.values()];
}
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, counts: {}, featured: [], assetReturns: [], strategyReturns: [], notice: "RETAIL database not configured." });
  }

  const [strategies, usersCount, reqActions, stockReturns, stratReturns] = await Promise.all([
    db.from("strategies_c").select("id, name, short_name, sector, risk_level, base_currency, is_public, is_featured, status, holdings").then((r) => r.data ?? []),
    db.from("profiles").select("id", { count: "exact", head: true }).then((r) => r.count ?? 0),
    db.from("required_actions").select("kyc_verified, bank_linked").then((r) => r.data ?? []),
    db.from("stock_returns_c").select("*").order("as_of_date", { ascending: false }).limit(3000).then((r) => r.data ?? []),
    db.from("strategies_returns_c").select("*").order("as_of_date", { ascending: false }).limit(3000).then((r) => r.data ?? []),
  ]);
  ...
  const strategyReturns: Record<string, unknown>[] = latestBy(stratReturns as Record<string, unknown>[], "strategy_id").map((r) => ({
    ...r,
    name: (stratById.get(r.strategy_id as string)?.name as string) || (r.strategy_id as string),
  }));

  const featured = strategies.filter((s) => s.is_featured).map((s) => ({
    id: s.id, name: s.name, short_name: s.short_name, sector: s.sector,
