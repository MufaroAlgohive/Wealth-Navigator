      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_280px]">
        <div className="space-y-5">
          ...
          {active === "uat" && canSeeUat && (
            <section>
              <CollapsibleCard title={`UAT strategy work · ${agendaCount} pending · ${approvedReqs.length} approved`} dataSource="supabase" db="institutional" open onToggle={() => undefined}>
                <p className="mb-3 text-caption">UAT research, votes and rebalance proposals are isolated from LIVE.</p>
                {agendaCount === 0 && approvedReqs.length === 0 && recent.length === 0 ? <p className="text-caption">No UAT research or strategy proposals yet.</p> : <div className="space-y-4">{visibleReqs.map((r) => <RebalanceAgendaItem key={r.id} req={r} code={rebCodes.get(r.id) ?? "REB"} linkedNote={linkedNoteForRebalance(r, agendaNotes)} canApprove={perms.approveRebalance} canApproveNote={perms.approveNote} canVote={perms.approveRebalance} viewerEmail={viewerEmail} pills={pills} onChanged={refresh} />)}{visibleNotes.map((n) => <ResearchAgendaItem key={n.id} note={n} linkedRebalance={linkedRebalanceForNote(n, pendingReqs)} perms={perms} viewerEmail={viewerEmail} pills={pills} onChanged={refresh} />)}{approvedReqs.length > 0 && <div className="border-t border-[hsl(var(--glass-border))] pt-4"><p className="mb-3 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Approved UAT rebalances · ready for Rebalance</p><div className="space-y-3">{approvedReqs.map((r) => <ApprovedItem key={r.id} req={r} code={rebCodes.get(r.id) ?? "REB"} canPush={perms.pushRebalance} onChanged={refresh} />)}</div></div>}{notes.filter((n) => n.status === "approved").length > 0 && <div className="border-t border-[hsl(var(--glass-border))] pt-4"><p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Approved UAT research</p><div className="flex flex-wrap gap-2">{notes.filter((n) => n.status === "approved").map((n) => <span key={n.id} className="rounded-md border border-[hsl(var(--up)/0.3)] bg-[hsl(var(--up)/0.08)] px-2 py-1 font-mono text-xs text-up">{n.symbol}</span>)}</div></div>}</div>}
              </CollapsibleCard>
            </section>
          )}
