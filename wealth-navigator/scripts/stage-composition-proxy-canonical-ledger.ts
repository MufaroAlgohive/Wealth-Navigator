...
const [compositions, batches, reconciliations, publications, existingRows] = await Promise.all([
  ...
  many<{
    ...
    db
      ...
      .select(
        "as_of_date,ledger_version,certification_status,securities_value_cents,continuity_cash_cents,complete_value_cents,period_metrics,source_evidence_sha256",
      )
      .eq("strategy_id", strategy.id),
  ),
]);
